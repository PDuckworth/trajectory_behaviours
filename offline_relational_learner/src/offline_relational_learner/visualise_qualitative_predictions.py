...
class qsr_param_masks(object):
	...
	def __init__(self, grid, dbg=False, colour_range=0, res=0.05):
		...
		print qsr_params, len(qsr_params)
		#Remove this when using only one QSR type

		if len(qsr_params) is 2:
			distance_rels, qtc_rels = qsr_params
			self.qsr = (distance_rels[0], qtc_rels[0])
			self.qsr_params = (distance_rels[1], qtc_rels[1])
		else:
			self.qsr, self.qsr_params = qsr_params

		self.binary_masks = {}
		self.cluster_occu = {}
		self.binary_map = self.map_listener()

		print "QSRs requested: ", self.qsr
		print self.qsr_params

		if "arg_distance" in self.qsr:
			rospy.loginfo('GENERATING QSR MASKS...')
			self.get_sorted_params()
			self.get_qsr_masks(on_screen=False)
	...
	def get_qsr_masks(self, on_screen=False):
	...
	def get_sorted_params(self):
		self.sorted_params = []
		print "sorted_params:", self.qsr_params
		if "qtcb" in self.qsr:
			qsr_params = self.qsr_params[0]
		else:
			qsr_params = self.qsr_params

		new_dict = dict (zip(qsr_params.values(), qsr_params.keys()))
		keys = new_dict.keys()
		keys.sort(key=int)
		for i in keys:
	...
	def get_graph_clusters_from_kmeans(self, estimator, code_book_hashes, code_book):
		...
		if "qtcb" in self.qsr: self.all_directional_mask = {}
		...
		for cnt, clst in enumerate(estimator.cluster_centers_):
			...
			occu, self.all_directional_mask[clstr_id] = self.generate_cluster_occupencyGrid(clstr_id, qsr_cluster_masks[clstr_id])
			t3 += time.time()-t
		...
		if self.dbg:
			...
			print "time4: ", t4
			print "time5: ", t5
			print "Cluster %s direction masks: %s" % (clstr_id, self.all_directional_mask[clstr_id])
	...
	def generate_cluster_occupencyGrid(self, cluster='', add_masks={}):
		...
		for cnt, ((object, map_xyz), list_of_mask_tuples) in enumerate(add_masks.items()):
			...
			for num_of_temporal_nodes, qsr_tuple in enumerate(list_of_mask_tuples):
				...
				if "arg_distance" in self.qsr and "qtcb" in self.qsr:
					modified_arg_dist = qsr[:-2]
					modified_qtc = qsr[-1]

					key = (object, self.objects[object])
					print "KEY: ", key
					sys.exit(1)
					if key in directions:
						directions[key].append(modified_qtc)
					else:
						directions[key] = [modified_qtc]
				else:
					modified_arg_dist = qsr

				occu.mat = o_utils.addAtPos(occu.mat, self.binary_masks[modified_arg_dist], pos, weight, add=True, vis=False)#vis=self.dbg)

			occu.matrix_to_occu(occu.mat)
		return occu, directions
