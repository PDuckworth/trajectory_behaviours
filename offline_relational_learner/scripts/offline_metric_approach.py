...
class OfflineMetricApproach(object):
...
def query_database_for_uuids():
    st_0 = time.time()
    metric = OfflineMetricApproach(vis = True)
    ...
    #metric.learn_threshold_from_first_days(t2_st)
    ...
    for i in range(int(loops)):
        ...
        #Keep accumulating the kept/rejected ids - used for vis
        #metric.uuids = []
        #metric.removed_ids = []
        ...
        #Already learnt GMM with 1 mean from day 1 data :)
        metric.filter_threshold =  0.01 #F1
        #metric.filter_threshold = 0.005 #F2
